"""
Standalone, sweep-ready extraction of 02_model_and_training_v4.ipynb.
Same classes/training logic as the notebook, byte-for-byte, with two additions:
  1. CLI hyperparameters, so a sweep can launch many configs as separate processes.
  2. `save_full_diagnostics=False` (default for sweep runs) skips the expensive
     collect_oscillator_states() + best_oscillator_states.npz dump that fires on
     every val-loss-improving epoch -- that dump is what made a full run take
     ~66s/epoch in benchmarking. Sweep configs only need best_hopf_model.pt +
     the loss history to pick a winner; the full oscillator-state dump is only
     produced for the final, chosen configuration (pass --full_diagnostics).
  3. Early stopping (patience on val loss) so a config that's stopped improving
     doesn't burn the rest of its epoch budget.
"""
import argparse
import json
import random
...
def train(output_dir, epochs=300, batch_size=32, learning_rate=1e-3, seed=42,
          ...
          pad_pre=20, pad_post=20, patience=40, save_full_diagnostics=False,
          device=None):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[{output_dir.name}] Epoch {epoch:04d} | Train {train_loss_epoch:.6f} | "
              f"Val {val_loss_epoch:.6f} | TF {tf_prob:.4f} | best {best_val_loss:.6f}@{best_epoch}",
              flush=True)

        if epochs_since_improve >= patience:
            print(f"[{output_dir.name}] Early stop at epoch {epoch} (no improvement in {patience} epochs)",
                  flush=True)
            break
    ...
    with open(output_dir / "metrics.json", "w") as f:
        json.dump({
            ...
            "epochs_run": epoch, "elapsed_sec": elapsed,
            "config": {
                "learning_rate": learning_rate, "batch_size": batch_size, "kappa": kappa,
    ...
    print(f"[{output_dir.name}] DONE best_val_loss={best_val_loss:.6f} @epoch {best_epoch} "
          f"({elapsed:.1f}s, {epoch} epochs)", flush=True)
...
if __name__ == "__main__":
    ...
    p.add_argument("--n_osc_per_side", type=int, default=25)
    p.add_argument("--v_hidden", type=int, default=16)
    p.add_argument("--readout_hidden", type=int, default=32)
    p.add_argument("--kappa", type=float, default=1.0)
    p.add_argument("--teacher_forcing_end", type=float, default=0.1)
    p.add_argument("--pad_pre", type=int, default=20)
    p.add_argument("--pad_post", type=int, default=20)
    p.add_argument("--patience", type=int, default=40)
    p.add_argument("--full_diagnostics", action="store_true")
    args = p.parse_args()
    ...
    train(
        ...
        teacher_forcing_end=args.teacher_forcing_end, pad_pre=args.pad_pre, pad_post=args.pad_post,
        patience=args.patience, save_full_diagnostics=args.full_diagnostics,
    )
